...
import { Plus, Minus, UserPlus, RotateCcw, Award, Menu, Save, Moon, Sun, Trash2, Edit, RefreshCw } from 'lucide-react';
...
const GameScoreTracker = () => {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} transition-colors duration-300`}>
      {/* Header */}
      <header className={`${darkMode ? 'bg-blue-800' : 'bg-blue-600'} p-4 sticky top-0 z-10 shadow-lg`}>
        <div className="flex justify-between items-center">
          ...
          <div className="text-center">
            <h1 className="font-bold text-xl text-white">Game Calc</h1>
          ...
          >
            ...
            <div className="flex p-4 pt-0 gap-2">
              ...
              <div className="flex rounded-lg overflow-hidden shadow-sm">
                <button 
                  className={`p-3 ${darkMode ? 'bg-gray-700' : 'bg-gray-200'} text-lg`}
      ...
      {/* Bottom action bar */}
      <div className={`fixed bottom-0 left-0 right-0 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg rounded-t-xl`}>
        <div className="flex justify-around items-center p-2">
          <button className="p-3 rounded-full hover:bg-gray-700/30 flex flex-col items-center">
            <UserPlus size={24} />
          ...
          <div className="relative -mt-8">
            <button className="w-16 h-16 rounded-full bg-blue-600 flex items-center justify-center shadow-lg hover:bg-blue-700 active:scale-95 transition-all">
              <Save size={28} />
            </button>
            <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs whitespace-nowrap font-medium">Save Round</span>
          </div>
          <button className="p-3 rounded-full hover:bg-gray-700/30 flex flex-col items-center">
            <Award size={24} />
